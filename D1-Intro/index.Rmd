...
Use R to add 615 to 247, then divided by 3, type `(615 + 247)/3` into the console then hitting Enter. What is the answer? 
...
tutorial::go_interactive(greedy=FALSE, height=200)
```
...
The data set called `MockJury` will be used as the example in Wednesday class. Let's try it out today. The data set is built within the `heplots` package. To begin, we need to install it the package (we just need to do it once!). Look at the bottom right pane. There you can choose *Packages* tab, and click *Install*. Type `heplots` when prompted and click ``Install``. That easy!
...
Instead of viewing the entire data set, you can just show the first six rows using `head`. Change back into the script and run. The output is long and it may be break into another block like below.
```{r head-data}
head(MockJury)
```
...
Each column in the data set is named (these named columns are called variables).  You can tell R to only print the "Years" variable using the dollar sign.
